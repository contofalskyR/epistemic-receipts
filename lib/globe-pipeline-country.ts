// Maps ingester pipeline tags to ISO 3166-1 alpha-2 country codes for pipelines
// with a known single-country origin. Multi-country pipelines (UN, EU, WHO,
// USGS, CrossRef, etc.) are omitted on purpose — their counts would be
// misleadingly attributed to one place.
export const PIPELINE_COUNTRY: Record<string, string> = {
  // United States
...
export const PIPELINE_COUNTRY_NAME: Record<string, string> = {
...
// Reverse lookup: country code → all pipeline tags ingesting for that country
export const COUNTRY_TO_PIPELINES: Record<string, string[]> = (() => {
  const out: Record<string, string[]> = {};
  for (const [pipeline, code] of Object.entries(PIPELINE_COUNTRY)) {
    (out[code] ||= []).push(pipeline);
  }
  return out;
})();
