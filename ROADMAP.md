...
- **Historical Event Graph (Phase 3)** — a `HistoricalEvent` entity (e.g. "Treaty of Versailles", "US Entry into WWI", "Nuremberg Trials") linked via many-to-many joins to `Claim`, `LegislativeVote`, and `Source`. Enables queries like: "What was the legislative consensus around X event? How many decision-makers voted on Y? What documents were produced?" Not all events will have formal vote counts — where they don't, the graph surfaces decision-makers and documents instead. Schema: `HistoricalEvent` table with `name`, `date`, `description`, `wikidata_id`; join tables `EventClaim`, `EventSource`, `EventLegislativeVote`. UI: event page showing vote tally (where available), linked primary sources, participating decision-makers. Prerequisite: Phase 2 schema (`result`, `topics` on `LegislativeVote`) should land first. Pairs naturally with the FRUS, JFK, NARA, and IPN archive pipelines already in DB.

- **Polity table + government type variable (Phase 3)** — a `Polity` table representing political entities with a lifespan: `id`, `countryCode` (ISO), `name` (e.g. "Nazi Germany", "Weimar Republic", "Soviet Union"), `governmentType` (enum: empire, republic, democracy, fascist, communist, authoritarian, monarchy, city-state, theocracy, etc.), `startYear`, `endYear` (null = still active). One territory can have multiple sequential polities (German Empire → Weimar → Nazi Germany → West/East Germany → unified Germany). `LegislativeVote`, `Claim`, and `HistoricalEvent` link to `Polity` by matching `countryCode` + year range — enabling queries like "What did fascist governments vote on?", "What claims exist about decisions under the Soviet Union?", "Show all votes on territorial expansion by empires 1880–1920." **Data source: Wikidata SPARQL** — a single query to `query.wikidata.org/sparql` can pull thousands of historical states with `instance of`, start/end dates, successor/predecessor links, and ISO country codes. The `instance of` values (hundreds of Wikidata types) need normalization into ~10–15 enum buckets — a one-time mapping pass. Coverage goes back to ancient history (negative `startYear` for BCE). This becomes the `governmentType` filter across all stats, votes, and claims.
