// Enrichment: epistemic arc for the FDA pravastatin sodium label claim.
//
// Claim: cmpiyl5uz91uuplo78nk5vdr2 (openfda_labels_v1)
//   PRAVASTATIN SODIUM — an HMG-CoA reductase inhibitor (statin) indicated to
//   reduce the risk of myocardial infarction, revascularization, and
//   cardiovascular mortality in adults with elevated LDL-C but without clinically
//   evident CHD (primary prevention), and to reduce the risk of coronary death,
//   MI, revascularization, and stroke/TIA in patients WITH clinically evident CHD
//   (secondary prevention).
//
// NOTE: A sibling script (enrich-corpus-openfda_labels_v1-pravastatin.ts) applies
// the same arc to a DIFFERENT pravastatin claim (cmpiyk0h090e6plo7vs4a8q3k). The
// Source rows are shared (identical externalIds) and upsert idempotently; only the
// ClaimStatusHistory slugs differ because they embed this claim's id.
//
...
//                         reduction, cementing broad clinical adoption. Ratified
//                         by INSTITUTIONAL (NIH/NHLBI expert panel).
...
//
// Run: npx tsx scripts/enrichments/enrich-corpus-openfda_labels_v1-pravastatin-sodium.ts
...
const CLAIM_ID = 'cmpiyl5uz91uuplo78nk5vdr2'
...
const TRANSITIONS: Transition[] = [
  ...
  },
  {
    ...
    datePrecision: 'DAY',
    reason:
      'The National Cholesterol Education Program (NCEP) Adult Treatment Panel III report, issued by the NIH National Heart, Lung, and Blood Institute, established LDL-lowering statin therapy as the standard of care for reducing coronary heart disease risk, defining LDL-C treatment targets and risk-based intensity across primary and secondary prevention. Its guideline endorsement drove broad clinical adoption of statins such as pravastatin for exactly the risk-reduction indications on this label, settling the therapeutic consensus.',
    source: {
  ...
  },
  {
    ...
    datePrecision: 'DAY',
    reason:
      'The FDA issued a Drug Safety Communication on 2012-02-28 requiring class-wide labeling changes for cholesterol-lowering statins, including pravastatin, adding warnings that statins can raise blood glucose and HbA1c (increased risk of incident diabetes) and can be associated with rare, generally reversible cognitive effects such as memory loss and confusion. Applying to the whole statin class, this post-market safety signal complicated the previously settled benefit consensus without withdrawing the drugs from market, moving the fact into a contested state.',
    source: {
      externalId: 'src:pravastatin-fda-statin-safety-2012',
      name: 'U.S. FDA. "FDA Drug Safety Communication: Important safety label changes to cholesterol-lowering statin drugs." 2012-02-28.',
