#!/usr/bin/env bash
# Perpetual loop for the slovenia legislation ingester.
...
LOG_FILE="/tmp/slovenia-loop.log"
SLEEP_HOURS=24
...
log "=== slovenia-loop started (sleep=${SLEEP_HOURS}h) ==="
...
while true; do
  ...
  npx dotenv-cli -e .env.local -- npx tsx scripts/ingest-slovenia-legislation.ts \
    --full \
