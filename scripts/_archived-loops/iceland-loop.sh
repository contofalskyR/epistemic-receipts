#!/usr/bin/env bash
# Perpetual loop for the iceland legislation ingester.
...
log "=== iceland-loop started (sleep=${SLEEP_HOURS}h) ==="
...
while true; do
  ...
  npx dotenv-cli -e .env.local -- npx tsx scripts/ingest-iceland-legislation.ts \
    --full \
