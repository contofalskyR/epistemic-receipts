#!/usr/bin/env bash
# Perpetual loop for the scotland legislation ingester.
...
LOG_FILE="/tmp/scotland-loop.log"
SLEEP_HOURS=24
...
log "=== scotland-loop started (sleep=${SLEEP_HOURS}h) ==="
...
while true; do
  ...
  npx dotenv-cli -e .env.local -- npx tsx scripts/ingest-scotland-legislation.ts \
    --full \
