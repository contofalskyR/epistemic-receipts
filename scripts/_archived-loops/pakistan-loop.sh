#!/usr/bin/env bash
# Perpetual loop for the pakistan legislation ingester.
...
LOG_FILE="/tmp/pakistan-loop.log"
SLEEP_HOURS=24
...
log "=== pakistan-loop started (sleep=${SLEEP_HOURS}h) ==="
...
while true; do
  ...
  npx dotenv-cli -e .env.local -- npx tsx scripts/ingest-pakistan-legislation.ts \
    --full \
