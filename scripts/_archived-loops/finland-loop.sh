#!/usr/bin/env bash
# Perpetual loop for the finland legislation ingester.
...
log "=== finland-loop started (sleep=${SLEEP_HOURS}h) ==="
...
while true; do
  ...
  npx dotenv-cli -e .env.local -- npx tsx scripts/ingest-finland-legislation.ts \
    --full \
