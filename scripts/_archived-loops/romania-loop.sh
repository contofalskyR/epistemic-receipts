#!/usr/bin/env bash
# Perpetual loop for the romania legislation ingester.
...
log "=== romania-loop started (sleep=${SLEEP_HOURS}h) ==="
...
while true; do
  ...
  npx dotenv-cli -e .env.local -- npx tsx scripts/ingest-romania-legislation.ts \
    --full \
