#!/usr/bin/env bash
# Perpetual loop for the chile legislation ingester.
...
log "=== chile-loop started (sleep=${SLEEP_HOURS}h) ==="
...
while true; do
  ...
  npx dotenv-cli -e .env.local -- npx tsx scripts/ingest-chile-legislation.ts \
    --full \
