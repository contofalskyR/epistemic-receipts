#!/usr/bin/env bash
# Perpetual loop for the mexico legislation ingester.
...
log "=== mexico-loop started (sleep=${SLEEP_HOURS}h) ==="
...
while true; do
  ...
  npx dotenv-cli -e .env.local -- npx tsx scripts/ingest-mexico-legislation.ts \
    --full \
