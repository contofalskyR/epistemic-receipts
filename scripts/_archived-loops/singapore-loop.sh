#!/usr/bin/env bash
# Perpetual loop for the singapore legislation ingester.
...
LOG_FILE="/tmp/singapore-loop.log"
SLEEP_HOURS=24
...
log "=== singapore-loop started (sleep=${SLEEP_HOURS}h) ==="
...
while true; do
  ...
  npx dotenv-cli -e .env.local -- npx tsx scripts/ingest-singapore-legislation.ts \
    --full \
