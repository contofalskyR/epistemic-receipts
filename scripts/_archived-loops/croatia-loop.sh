#!/usr/bin/env bash
# Perpetual loop for the croatia legislation ingester.
...
log "=== croatia-loop started (sleep=${SLEEP_HOURS}h) ==="
...
while true; do
  ...
  npx dotenv-cli -e .env.local -- npx tsx scripts/ingest-croatia-legislation.ts \
    --full \
