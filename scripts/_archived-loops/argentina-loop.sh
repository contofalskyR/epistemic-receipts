#!/usr/bin/env bash
# Perpetual loop for the argentina legislation ingester.
...
LOG_FILE="/tmp/argentina-loop.log"
SLEEP_HOURS=24
...
log "=== argentina-loop started (sleep=${SLEEP_HOURS}h) ==="
...
while true; do
  ...
  npx dotenv-cli -e .env.local -- npx tsx scripts/ingest-argentina-legislation.ts \
    --full \
