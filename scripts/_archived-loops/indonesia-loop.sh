#!/usr/bin/env bash
# Perpetual loop for the indonesia legislation ingester.
...
LOG_FILE="/tmp/indonesia-loop.log"
SLEEP_HOURS=24
...
log "=== indonesia-loop started (sleep=${SLEEP_HOURS}h) ==="
...
while true; do
  ...
  npx dotenv-cli -e .env.local -- npx tsx scripts/ingest-indonesia-legislation.ts \
    --full \
