#!/usr/bin/env bash
# Perpetual loop for the bulgaria legislation ingester.
...
LOG_FILE="/tmp/bulgaria-loop.log"
SLEEP_HOURS=24
...
log "=== bulgaria-loop started (sleep=${SLEEP_HOURS}h) ==="
...
while true; do
  ...
  npx dotenv-cli -e .env.local -- npx tsx scripts/ingest-bulgaria-legislation.ts \
    --full \
