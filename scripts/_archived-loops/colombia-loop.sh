#!/usr/bin/env bash
# Perpetual loop for the colombia legislation ingester.
...
LOG_FILE="/tmp/colombia-loop.log"
SLEEP_HOURS=24
...
log "=== colombia-loop started (sleep=${SLEEP_HOURS}h) ==="
...
while true; do
  ...
  npx dotenv-cli -e .env.local -- npx tsx scripts/ingest-colombia-legislation.ts \
    --full \
