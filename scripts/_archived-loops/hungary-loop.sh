#!/usr/bin/env bash
# Perpetual loop for the hungary legislation ingester.
...
log "=== hungary-loop started (sleep=${SLEEP_HOURS}h) ==="
...
while true; do
  ...
  npx dotenv-cli -e .env.local -- npx tsx scripts/ingest-hungary-legislation.ts \
    --full \
