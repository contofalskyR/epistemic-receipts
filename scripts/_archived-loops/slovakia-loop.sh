#!/usr/bin/env bash
# Perpetual loop for the slovakia legislation ingester.
...
LOG_FILE="/tmp/slovakia-loop.log"
SLEEP_HOURS=24
...
log "=== slovakia-loop started (sleep=${SLEEP_HOURS}h) ==="
...
while true; do
  ...
  npx dotenv-cli -e .env.local -- npx tsx scripts/ingest-slovakia-legislation.ts \
    --full \
