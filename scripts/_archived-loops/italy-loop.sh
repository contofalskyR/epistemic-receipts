#!/usr/bin/env bash
# Perpetual loop for the italy legislation ingester.
...
log "=== italy-loop started (sleep=${SLEEP_HOURS}h) ==="
...
while true; do
  ...
  npx dotenv-cli -e .env.local -- npx tsx scripts/ingest-italy-legislation.ts \
    --full \
