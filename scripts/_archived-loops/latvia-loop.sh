#!/usr/bin/env bash
# Perpetual loop for the latvia legislation ingester.
...
log "=== latvia-loop started (sleep=${SLEEP_HOURS}h) ==="
...
while true; do
  ...
  npx dotenv-cli -e .env.local -- npx tsx scripts/ingest-latvia-legislation.ts \
    --full \
