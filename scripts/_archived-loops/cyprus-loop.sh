#!/usr/bin/env bash
# Perpetual loop for the cyprus legislation ingester.
...
log "=== cyprus-loop started (sleep=${SLEEP_HOURS}h) ==="
...
while true; do
  ...
  npx dotenv-cli -e .env.local -- npx tsx scripts/ingest-cyprus-legislation.ts \
    --full \
