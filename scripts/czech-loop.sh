#!/usr/bin/env bash
# Perpetual loop for the czech legislation ingester.
...
log "=== czech-loop started (sleep=${SLEEP_HOURS}h) ==="
...
while true; do
  ...
  npx dotenv-cli -e .env.local -- npx tsx scripts/ingest-czech-legislation.ts \
    --full \
