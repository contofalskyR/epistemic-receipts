#!/usr/bin/env bash
# Perpetual loop for the malaysia legislation ingester.
...
LOG_FILE="/tmp/malaysia-loop.log"
SLEEP_HOURS=24
...
log "=== malaysia-loop started (sleep=${SLEEP_HOURS}h) ==="
...
while true; do
  ...
  npx dotenv-cli -e .env.local -- npx tsx scripts/ingest-malaysia-legislation.ts \
    --full \
