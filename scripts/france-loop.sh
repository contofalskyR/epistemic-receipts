#!/usr/bin/env bash
# Perpetual loop for the france legislation ingester.
...
log "=== france-loop started (sleep=${SLEEP_HOURS}h) ==="
...
while true; do
  ...
  npx dotenv-cli -e .env.local -- npx tsx scripts/ingest-france-legislation.ts \
    --full \
