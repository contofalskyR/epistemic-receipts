#!/usr/bin/env bash
# Perpetual loop for the thailand legislation ingester.
...
LOG_FILE="/tmp/thailand-loop.log"
SLEEP_HOURS=24
...
log "=== thailand-loop started (sleep=${SLEEP_HOURS}h) ==="
...
while true; do
  ...
  npx dotenv-cli -e .env.local -- npx tsx scripts/ingest-thailand-legislation.ts \
    --full \
