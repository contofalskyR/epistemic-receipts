#!/usr/bin/env bash
# Perpetual loop for the russia legislation ingester.
...
log "=== russia-loop started (sleep=${SLEEP_HOURS}h) ==="
...
while true; do
  ...
  npx dotenv-cli -e .env.local -- npx tsx scripts/ingest-russia-legislation.ts \
    --full \
