#!/usr/bin/env bash
# Loop 5: Settling curve auto-expander
# Runs every 4 hours via launchd. Rotates focus area so each run targets different eras/regions.
...
# Rotate focus area based on hour-of-day to avoid repetition
HOUR=$(date +%H)
case $((10#$HOUR / 4)) in
...
*) FOCUS="Modern era (1990–present): Internet, genomics, climate science, 9/11 aftermath, COVID-19, AI. Non-Western perspectives prioritized." ;;
esac

log "=== settling-curve-loop start. Focus: $FOCUS ==="

# Get current trajectory count to measure growth
...
log "Added: ${ADDED:-0} trajectories"

if [ "${ADDED:-0}" != "0" ] && [ "${ADDED:-0}" != "" ]; then
  bash "$NOTIFY" "🌊 Settling curve expanded: +${ADDED} new trajectories
📚 ${TITLES}
Total trajectories was ${CURRENT_COUNT} claimIds"
fi
