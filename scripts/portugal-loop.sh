#!/usr/bin/env bash
# Perpetual loop for the portugal legislation ingester.
...
LOG_FILE="/tmp/portugal-loop.log"
SLEEP_HOURS=24
...
log "=== portugal-loop started (sleep=${SLEEP_HOURS}h) ==="
...
while true; do
  ...
  npx dotenv-cli -e .env.local -- npx tsx scripts/ingest-portugal-legislation.ts \
    --full \
