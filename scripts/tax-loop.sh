#!/usr/bin/env bash
# Runs CourtListener Tax Court + Court of Federal Claims ingester on a continuous loop.
...
log "=== tax-loop started (limit=$LIMIT, min-citations=$MIN_CITATIONS, sleep=${SLEEP_HOURS}h) ==="

cd "$PROJECT_DIR"

while true; do
  log "--- Starting pass ---"
  npx dotenv-cli -e .env.local -- npx tsx scripts/ingest-courtlistener-tax.ts \
    --slow \
    --limit "$LIMIT" \
