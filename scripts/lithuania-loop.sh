#!/usr/bin/env bash
# Perpetual loop for the lithuania legislation ingester.
...
LOG_FILE="/tmp/lithuania-loop.log"
SLEEP_HOURS=24
...
log "=== lithuania-loop started (sleep=${SLEEP_HOURS}h) ==="
...
while true; do
  ...
  npx dotenv-cli -e .env.local -- npx tsx scripts/ingest-lithuania-legislation.ts \
    --full \
