#!/usr/bin/env bash
# Perpetual loop for the australia legislation ingester.
...
LOG_FILE="/tmp/australia-loop.log"
SLEEP_HOURS=24
...
log "=== australia-loop started (sleep=${SLEEP_HOURS}h) ==="
...
while true; do
  ...
  npx dotenv-cli -e .env.local -- npx tsx scripts/ingest-australia-legislation.ts \
    --full \
