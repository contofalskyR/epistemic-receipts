#!/usr/bin/env bash
# Perpetual loop for the spain legislation ingester.
...
log "=== spain-loop started (sleep=${SLEEP_HOURS}h) ==="
...
while true; do
  ...
  npx dotenv-cli -e .env.local -- npx tsx scripts/ingest-spain-legislation.ts \
    --full \
