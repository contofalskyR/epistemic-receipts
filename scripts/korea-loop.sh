#!/usr/bin/env bash
# Perpetual loop for the korea legislation ingester.
...
log "=== korea-loop started (sleep=${SLEEP_HOURS}h) ==="
...
while true; do
  ...
  npx dotenv-cli -e .env.local -- npx tsx scripts/ingest-korea-legislation.ts \
    --full \
