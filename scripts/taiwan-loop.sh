#!/usr/bin/env bash
# Perpetual loop for the taiwan legislation ingester.
...
log "=== taiwan-loop started (sleep=${SLEEP_HOURS}h) ==="
...
while true; do
  ...
  npx dotenv-cli -e .env.local -- npx tsx scripts/ingest-taiwan-legislation.ts \
    --full \
