#!/usr/bin/env bash
# Perpetual loop for the malta legislation ingester.
...
log "=== malta-loop started (sleep=${SLEEP_HOURS}h) ==="
...
while true; do
  ...
  npx dotenv-cli -e .env.local -- npx tsx scripts/ingest-malta-legislation.ts \
    --full \
