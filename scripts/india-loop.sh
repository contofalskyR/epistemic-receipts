#!/usr/bin/env bash
# Perpetual loop for the india legislation ingester.
...
log "=== india-loop started (sleep=${SLEEP_HOURS}h) ==="
...
while true; do
  ...
  npx dotenv-cli -e .env.local -- npx tsx scripts/ingest-india-legislation.ts \
    --full \
