#!/usr/bin/env bash
# Perpetual loop for the poland legislation ingester.
...
log "=== poland-loop started (sleep=${SLEEP_HOURS}h) ==="
...
while true; do
  ...
  npx dotenv-cli -e .env.local -- npx tsx scripts/ingest-poland-legislation.ts \
    --full \
