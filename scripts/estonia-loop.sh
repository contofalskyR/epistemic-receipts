#!/usr/bin/env bash
# Perpetual loop for the estonia legislation ingester.
...
log "=== estonia-loop started (sleep=${SLEEP_HOURS}h) ==="
...
while true; do
  ...
  npx dotenv-cli -e .env.local -- npx tsx scripts/ingest-estonia-legislation.ts \
    --full \
