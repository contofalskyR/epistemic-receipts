#!/usr/bin/env bash
# Perpetual loop for the eu legislation ingester.
...
log "=== eu-loop started (sleep=${SLEEP_HOURS}h) ==="
...
while true; do
  ...
  npx dotenv-cli -e .env.local -- npx tsx scripts/ingest-eu-legislation.ts \
    --full \
