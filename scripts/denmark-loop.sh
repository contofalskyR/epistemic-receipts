#!/usr/bin/env bash
# Perpetual loop for the denmark legislation ingester.
...
log "=== denmark-loop started (sleep=${SLEEP_HOURS}h) ==="
...
while true; do
  ...
  npx dotenv-cli -e .env.local -- npx tsx scripts/ingest-denmark-legislation.ts \
    --full \
