#!/usr/bin/env bash
# Perpetual loop for the belgium legislation ingester.
...
log "=== belgium-loop started (sleep=${SLEEP_HOURS}h) ==="
...
while true; do
  ...
  npx dotenv-cli -e .env.local -- npx tsx scripts/ingest-belgium-legislation.ts \
    --full \
