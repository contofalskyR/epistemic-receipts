#!/usr/bin/env bash
# Perpetual loop for the norway legislation ingester.
...
log "=== norway-loop started (sleep=${SLEEP_HOURS}h) ==="
...
while true; do
  ...
  npx dotenv-cli -e .env.local -- npx tsx scripts/ingest-norway-legislation.ts \
    --full \
