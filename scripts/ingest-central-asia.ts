// Pipeline 99 — Central Asia Legislation (central_asia_v1)
// Dataset: Official legal information portals for Central Asian and South Caucasus nations.
//
// Country coverage & strategy:
//   Armenia     — arlis.am           — CDX enumerate + Wayback fetch (Angular SPA, no API found).
//   Azerbaijan  — e-qanun.az         — CDX enumerate + Wayback fetch (Next.js app, no public API).
//   Uzbekistan  — lex.uz             — CDX enumerate + Wayback fetch (ASP.NET, law list not browsable).
//   Kyrgyzstan  — cbd.minjust.gov.kg — CDX enumerate + Wayback fetch (403 blocked on direct access).
//
// Language note: titles are in Armenian, Azerbaijani, Uzbek, and Kyrgyz respectively.
// The CDX-based approach captures page titles as-is from archived snapshots.
//
// Run:
//   npx dotenv-cli -e .env.local -- npx tsx scripts/ingest-central-asia.ts --dry-run
//   npx dotenv-cli -e .env.local -- npx tsx scripts/ingest-central-asia.ts --sample 10
//   npx dotenv-cli -e .env.local -- npx tsx scripts/ingest-central-asia.ts --full [--verbose]
...
const INGESTED_BY = 'central_asia_v1'
const PIPELINE = 'Pipeline 99'
...
const COUNTRIES: Country[] = [
  {
    ...
    cdxPattern: 'arlis.am/*',
    // arlis.am document pages: /DocumentView.aspx?DocID=N or hash-based
    slugRegex: /arlis\.am\/(?:hy|ru|en)\/document\/([^/?#]+)/i,
  },
  {
    code: 'az',
    name: 'Azerbaijan',
    language: 'Azerbaijani (az)',
    cdxPattern: 'e-qanun.az/*',
    // e-qanun.az: /framework/document/{slug} or /document/{id}
    slugRegex: /e-qanun\.az\/(?:framework\/document|document)\/([^/?#]+)\/?$/i,
  },
  {
    code: 'uz',
    name: 'Uzbekistan',
    language: 'Uzbek (uz)',
    cdxPattern: 'lex.uz/docs/*',
    // lex.uz: /docs/{id} — the ASP.NET legislation portal
    slugRegex: /lex\.uz\/docs\/(-?\d+)(?:[?#]|$)/i,
  },
  {
    code: 'kg',
    name: 'Kyrgyzstan',
    language: 'Kyrgyz (ky)',
    cdxPattern: 'cbd.minjust.gov.kg/*',
    // cbd.minjust.gov.kg: law pages at /act/{id}
    slugRegex: /minjust\.gov\.kg\/(?:act|zakon|law)\/([^/?#]+)\/?$/i,
  },
]
    ...
    const req = (lib as typeof https).request(
      {
        ...
        headers: {
          ...
          'Accept-Language': 'en,hy,az,uz,ky;q=0.8',
...
async function getCdxSlugs(country: Country): Promise<ActSlug[]> {
  ...
  for (const row of data.slice(1)) {
    ...
    if (!slug || slug.length < 1 || seen.has(slug)) continue
    seen.add(slug)
...
function extractTitle(html: string, countryCode: string): string {
  ...
  if (ogTitle && ogTitle[1]!.trim().length > 3) return decodeHtmlEntities(ogTitle[1]!)
  ...
  t = t.replace(/\s*[-|–]\s*(ARLIS|e-Qanun|Lex\.uz|CBD|MinJust|Qonunchilik).*/i, '')
  t = t.replace(/\s*[-|–]\s*(Azerbaijan|Armenia|Uzbekistan|Kyrgyzstan).*/i, '')
  return t.trim()
}
...
async function main() {
  ...
  console.log(`\n── ${PIPELINE}: Central Asia Legislation ──`)
  console.log(`Mode: ${mode}${mode === 'sample' ? ` (n=${sampleN})` : ''}`)
  console.log(`Countries: Armenia, Azerbaijan, Uzbekistan, Kyrgyzstan`)
  console.log(`Source: All via Wayback CDX enumeration (JS SPAs / WAF blocked)`)
  console.log(`Language note: titles in Armenian, Azerbaijani, Uzbek, Kyrgyz as archived\n`)
  ...
  for (const country of COUNTRIES) {
    process.stdout.write(`  ${country.name} (${country.code}) [${country.language}]... `)
  ...
  if (totalActs === 0) {
    console.warn('WARNING: 0 acts enumerated — CDX coverage may be sparse for these sites.')
    console.warn('These sites are primarily JS-rendered SPAs with low Wayback crawl rates.')
  }
  ...
  if (mode === 'dry-run') {
    ...
    const output = {
      ...
      countsByCountry,
      notes: {
        coverage: 'All 4 sites are JavaScript SPAs or WAF-protected. CDX coverage varies — sparse for arlis.am, moderate for lex.uz.',
        languages: 'Titles ingested as-is in native language. No transliteration applied.',
        kyrgyzstan: 'cbd.minjust.gov.kg returns 403 on direct access — CDX-only.',
      },
    ...
    fs.writeFileSync('pipeline-99-dry-run-sample.json', JSON.stringify(output, null, 2))
    console.log('\nWritten: pipeline-99-dry-run-sample.json')
  ...
  for (const country of COUNTRIES) {
    const id = await ensureTopic(
      `${country.code}-legislature`,
      `Legislature of ${country.name}`,
      'government',
      'gov-region-asia'
    )
