#!/usr/bin/env bash
# Perpetual loop for the serbia legislation ingester.
...
log "=== serbia-loop started (sleep=${SLEEP_HOURS}h) ==="
...
while true; do
  ...
  npx dotenv-cli -e .env.local -- npx tsx scripts/ingest-serbia-legislation.ts \
    --full \
