#!/usr/bin/env bash
# Perpetual loop for the japan legislation ingester.
...
log "=== japan-loop started (sleep=${SLEEP_HOURS}h) ==="
...
while true; do
  ...
  npx dotenv-cli -e .env.local -- npx tsx scripts/ingest-japan-legislation.ts \
    --full \
