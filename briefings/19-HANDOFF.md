...
**UPDATE (end of session): §0's backfill is DONE AND VERIFIED.** Set-based
executor ran clean: 1,579,951 scanned, **796,026 updated** (+29,777 from the
Dispatch partial = 825,803, exactly the dry-run total); verification pass
(`--fresh`, no --execute) = **would-update 0 corpus-wide**. Robert was then
given the merge (`git checkout main && git merge fable/ctgov-follow-axis &&
git push`) — VERIFY whether it ran (git log/branch + Vercel prod deploy).
If merged: resume at STOP #2 (§2 step 5). If not: the merge is the next
command. Everything below §0 stands.
