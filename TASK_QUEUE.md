...
- [ ] ⏸ **NARA Catalog API key exposed in tracked files** — flagged by worker 2026-06-10 during NYT-key re-verification. The same key `KSHVEuDXNd27xXkByehli5Eak8TvnKJi99Kiz7DK` is hardcoded in `ROADMAP.md:227`, `NARA-ROADMAP.md:5`, and `HISTORY.md:69` (third location found 2026-06-14). Free key (10k calls/mo, register at Catalog_API@nara.gov) so low severity, but it is a live credential committed in plaintext. Human-only rotation: regenerate at the NARA Catalog API portal, then move the value to `.env.local` as `NARA_API_KEY` and replace the three doc references with `process.env.NARA_API_KEY` / a redacted placeholder. Out of scope for the NYT/Azure rotation task; surfaced here so it isn't lost.
