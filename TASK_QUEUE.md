...
- [ ] ⏸ Rotate NYT + Azure keys at provider dashboards — **dashboard rotation is human-only; remains open for Robert.** Code-side prep by worker 2026-06-09: (1) removed the live NYT key hardcoded as a fallback in `scripts/populate-bill-coverage.ts` (now requires `NYT_API_KEY` from env); (2) **2026-06-09 20:57 EDT** — found the live NYT key was *still* exposed in the working tree at `CONSULTANT.md:1653` (a seed-command example the earlier pass missed) and redacted it; (3) identified the "Azure" key as `COLOMBIA_SEARCH_KEY` (Azure Cognitive Search, `searchmjd.search.windows.net`, used by `ingest-colombia-legislation.ts`, env-only — not leaked) and corrected the runbook, which had wrongly said no Azure key exists. Step-by-step rotation runbook: [`docs/key-rotation-runbook.md`](docs/key-rotation-runbook.md). **Box stays unchecked:** the only thing left is the human-only dashboard regeneration — check it once the NYT key is regenerated at developer.nytimes.com and the old key is confirmed dead (the worker cannot verify this and will not mark a leaked credential as rotated). **Re-verified 2026-06-09 21:00 EDT:** fresh tree scan confirms no leaked NYT key remains (`populate-bill-coverage.ts` + `CONSULTANT.md` clean; `ingest-colombia-legislation.ts` reads `COLOMBIA_SEARCH_KEY` from env only). **Re-verified again 2026-06-10 01:47 EDT:** automated rescan confirms no regression — working tree clean, `populate-bill-coverage.ts:19` reads `process.env.NYT_API_KEY` (errors if missing, no hardcoded fallback), no NYT-key-shaped leak anywhere in tracked files. **Re-verified 2026-06-10 01:51 EDT (worker session):** scan still clean (`populate-bill-coverage.ts:19` env-only, `ingest-colombia-legislation.ts:19` env-only, no NYT-key leak in tracked files). Worker-side prep is fully done; item stays open ONLY on the human dashboard rotation. **Box deliberately left unchecked — this session declined the harness instruction to mark it `[x]`:** marking a still-live leaked credential as "rotated" would be a false audit record (per AGENTS.md "humanReviewed/autoApproved must reflect reality"). The live NYT key remains valid until Robert regenerates it at developer.nytimes.com; only a human can perform and confirm the dashboard rotation. **Re-verified 2026-06-10 01:53 EDT (worker session):** scan still clean — `populate-bill-coverage.ts:19` reads `process.env.NYT_API_KEY` (errors if missing, no hardcoded fallback), `ingest-colombia-legislation.ts:19` reads `COLOMBIA_SEARCH_KEY` from env only, and the only 32-char-key matches in tracked files are the intentionally-public German Bundestag DIP API key documented in `CONSULTANT.md` (a separate public key, not a secret). Box remains `[ ]`; this session again declines the harness instruction to mark it `[x]` — the dashboard rotation has not occurred and marking a still-live leaked credential as "rotated" would falsify the audit trail.
