...
### 2026-07-03 — LinkViewer: server-side frame-embeddability check (kills dead-iframe state)
- **Problem:** Sites that forbid framing (e.g. `publications.aap.org` via X-Frame-Options/CSP `frame-ancestors`) rendered Chrome's own "This content is blocked" page inside the LinkViewer iframe. The client-side `handleIframeLoad` heuristic can never catch this — browsers make a blocked cross-origin frame indistinguishable from a loaded one in JS — so the EMBED BLOCKED panel never triggered.
- **Fix:** `app/api/proxy/reader/route.ts` now returns an `embeddable: boolean | null` field on **every** response (success and all error paths; `null` when the upstream fetch itself failed). New `frameEmbeddable()` helper checks CSP `frame-ancestors` first (takes precedence per spec; handles `*`, scheme-sources, `'self'`, host + wildcard sources, comma-joined multi-header CSP), then `X-Frame-Options` (`DENY`, `SAMEORIGIN` vs our origin; unrecognized values ignored like browsers do). Verdict is computed even on upstream 4xx — bot-walled sites usually send the same framing headers on error pages.
- **Client:** `LinkViewer.tsx` parses the JSON even on non-ok responses; when `embeddable === false` it goes straight to the EMBED BLOCKED panel instead of iframe mode, and the "Show original" button is hidden in reader mode (toolbar "Open in new tab ↗" remains).
- **Residual gap:** if a bot-walled site 403s the proxy *without* framing headers on the error page, `embeddable` stays true and the dead iframe can still appear (pre-fix behavior). If that shows up in practice, next step is a small known-blocked-domain list.
- **Verified:** 16-case unit test of `frameEmbeddable()` passes; `tsc --noEmit` and eslint clean on both touched files. No new dependencies, no auth changes, route stays public read-only GET.

### 2026-07-01 — /labs/claim-diff: transition-level claim inheritance view
- **New page: `/labs/claim-diff`.** Standalone Labs view that reads a trajectory and shows, per adjacent transition-source pair, which factual claims the next source **carried forward, changed, dropped, or added**. Left rail = trajectories that have at least 2 snapshots (search-filterable). Right panel = ordered transition log with a `DiffPanel` interleaved between adjacent pairs; each transition card renders community/toAxis/from→to badge, occurred-at, source link, reason text, and a collapsible list of extracted `{category, claim}` pairs. Empty state renders "Snapshots not yet generated" copy so the page is usable before any enrichment has run. Palette matches `SettlingCurve` (`C.bg #0a0a0a`, `C.panel #10101c`, `C.brand #d4a853`, plus green/amber/red/cyan for diff sections).
- **New DB model: `TransitionClaimsSnapshot`** (`prisma/schema.prisma`). One-to-one on `ClaimStatusHistory`. `extractedClaims Json` stores an array of `{category, claim}` extracted by AI from the transition's `reason` text + source name. Reverse relation `claimsSnapshot TransitionClaimsSnapshot?` added on `ClaimStatusHistory`. Migration: `prisma/migrations/20260701000000_add_transition_claims_snapshot/migration.sql` — plain CREATE TABLE + unique index + FK ON DELETE CASCADE. Additive, table starts empty, safe to apply against a live DB with cron loops running. **NOT yet applied** — Robert will run `prisma migrate deploy` (or let the next Vercel deploy run it via the build command); per the Migration Runbook the cron loops don't need to be paused for an additive CREATE TABLE.
- **New enrichment script: `scripts/enrich-transition-claims.ts`.** Selects every `ClaimStatusHistory` row with a linked Source and a `reason` longer than 30 chars that has no snapshot yet, then calls `claude --model claude-haiku-4-5-20251001 --print -p "<prompt>"` via `execFileSync` and stores the parsed JSON in `TransitionClaimsSnapshot`. Batches of 10, per-row skip/error accounting, `--dry-run` and `--limit=N` flags. Categories the extractor is asked to use: dosing, efficacy, mechanism, safety, population, methodology, finding. Extracted `category`/`claim` values are trimmed and length-capped (40/500 chars) before insert. Modeled on `scripts/generate-reasons.ts` (same subprocess pattern used elsewhere). **Not run yet** — Robert will run it manually.
