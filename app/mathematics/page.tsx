...
import type { ColorKey, Family, MathEntry, Section } from "./types";
...
const SECTION_INFO: Record<Section, { name: string; tagline: string }> = {
  A: { name: "Section A — Foundations", tagline: "The logical and set-theoretic ground of mathematics." },
  B: { name: "Section B — Pure Mathematics", tagline: "Structure, number, space, and quantity studied for their own sake." },
  C: { name: "Section C — Applied & Computational", tagline: "Mathematics built to model, compute, decide, and communicate." },
  D: { name: "Section D — Famous Theorems & Open Problems", tagline: "Landmark results and the field's great open problems." },
};
...
type EntryRef = { entry: MathEntry; family: Family };
...
function entryMatches(entry: MathEntry, query: string): boolean {
  if (!query) return true;
  const q = query.toLowerCase();
  if (entry.name.toLowerCase().includes(q)) return true;
  if (entry.description.toLowerCase().includes(q)) return true;
  if (plainText(entry.statement).toLowerCase().includes(q)) return true;
...
function layoutFamilyGraph(family: Family): { nodes: GraphNode[]; edges: GraphEdge[] } {
  ...
  // Any remaining (cycles or unreached) get level 0
...
// ────────────────────────────────────────────────────────────────────────────
// Status badge for theorems / open problems
// ────────────────────────────────────────────────────────────────────────────

function StatusBadge({ entry }: { entry: MathEntry }) {
  if (!entry.status) return null;
  if (entry.status === "open") {
    return (
      <span className="text-[10px] px-1.5 py-0.5 rounded bg-red-950/60 text-red-300 font-mono border border-red-900/60">
        OPEN{entry.millennium ? " · millennium" : ""}
      </span>
    );
  }
  return (
    <span className="text-[10px] px-1.5 py-0.5 rounded bg-green-950/60 text-green-300 font-mono border border-green-900/60">
      SOLVED{entry.millennium ? " · millennium" : ""}
    </span>
  );
}

function XrefBadges({ entry }: { entry: MathEntry }) {
  ...
  return (
    ...
    >
      ...
      <div className="mt-2 text-xs text-gray-300 leading-relaxed">
        <span className="text-[10px] uppercase tracking-widest text-gray-500 mr-2">Statement</span>
        <MathFragment text={entry.statement} />
      </div>
                ...
                {entry.prereqs.map((p) => {
                  ...
                  if (ref) {
                    return (
                      <li key={p}>
                        <Link
                          href={`/mathematics#${slugifyEntry(ref.entry.name)}`}
                          onClick={(e) => e.stopPropagation()}
...
// Precompute "builds toward" index: for each entry name, list of entries that have it as a prereq.
...
export default function MathematicsPage() {
  ...
  const bySection: Record<Section, { family: Family; entries: MathEntry[] }[]> = { A: [], B: [], C: [], D: [] };
  for (const f of filtered) bySection[f.family.section].push(f);

  const sectionCounts: Record<Section, number> = { A: 0, B: 0, C: 0, D: 0 };
  for (const fam of ALL_FAMILIES) sectionCounts[fam.section] += fam.entries.length;

  return (
    <div className="space-y-8">
      <div className="border-b border-gray-800 pb-6">
        <h1 className="text-2xl font-semibold text-white">Mathematics — A Working Taxonomy</h1>
        <p className="mt-3 text-sm text-gray-400 leading-relaxed">
          A field guide to mathematics organized by family — from logic and foundations through pure
          and applied subfields to famous theorems and open problems. Each card carries the core
          <em> statement</em> (LaTeX, typeset with KaTeX), <em>key results</em>, and a
          <em> prerequisite</em> list — the directed edges of a dependency DAG you can toggle on
          per family. Color codes the section; clicking a header collapses it; clicking a card
          expands it. <span className="text-gray-300">Open problems are tagged <span className="text-red-400">OPEN</span> and solved landmarks <span className="text-green-400">SOLVED</span>.</span>
        </p>
        <p className="mt-3 text-xs text-gray-500 leading-relaxed">
          Cross-references: probability and parts of optimization, information, and game/decision
          theory overlap heavily with{" "}
          ...
          Whether statistics or theoretical CS is &ldquo;a branch of mathematics,&rdquo; whether category
          theory belongs under foundations, and the &ldquo;pure&rdquo; vs &ldquo;applied&rdquo; split are
          genuinely disputed; this page states the dispute and does not adjudicate.
        </p>
      ...
      <div className="sticky top-0 z-10 -mx-6 px-6 py-3 bg-gray-950/95 backdrop-blur border-b border-gray-800/60 flex flex-col sm:flex-row gap-3 sm:items-center">
        <input
          ...
          placeholder="Filter by name, description, statement, tag — e.g. 'compact', 'Galois', 'NP', 'prime'"
      ...
      <div className="border-t border-gray-800 pt-6 mt-12 space-y-3">
        <p className="text-xs text-gray-500 leading-relaxed">
          ...
          explorer that links mathematical concepts to the specific receipts that cite them is on
          the roadmap, as is a site-wide prerequisite knowledge map across all 246 entries.
        </p>
        <p className="text-xs text-gray-500 leading-relaxed">
          <span className="text-gray-400">Accuracy note:</span> open / solved status verified as of
          early 2026 — only the <strong>Poincaré conjecture</strong> is solved among the seven
          Millennium Prize Problems; the other six (Riemann Hypothesis, P vs NP, Navier–Stokes,
          Hodge, Yang–Mills, Birch &amp; Swinnerton-Dyer) and Goldbach, twin-prime, and Collatz
          remain open. Items with uncertain hypotheses are flagged for review rather than
          fabricated. Reports of inaccuracy welcome via the{" "}
          <Link href="/feedback" className="underline underline-offset-2">feedback</Link> page.
        </p>
        <p className="text-xs font-mono text-gray-700">
          taxonomy curated 2026-06-04 · LaTeX typesetting via KaTeX · prerequisite DAG built from entry data ·
