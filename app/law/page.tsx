...
import type { LawEntry, ColorKey, Family, Section } from "./types";
...
const SECTION_INFO: Record<Section, { name: string; tagline: string }> = {
  A: { name: "Section A — Constitutional & Public Law", tagline: "Foundational structure of the state — constitutional law, administrative law, taxation, and bankruptcy." },
  B: { name: "Section B — Criminal Law & Procedure", tagline: "Substantive criminal law, criminal procedure, and the rules of evidence." },
  C: { name: "Section C — Civil & Private Law", tagline: "Torts, contracts, property, family, civil procedure, business organizations, and intellectual property." },
  D: { name: "Section D — International, Comparative & Cross-cutting", tagline: "International law, human rights, environmental, labor, immigration, and comparative legal traditions." },
  E: { name: "Section E — Legal Theory & Landmark Cases", tagline: "Jurisprudence, philosophy of law, and the landmark decisions and open questions that shape the field." },
};
...
function entryMatches(entry: LawEntry, query: string): boolean {
...
function XrefBadges({ entry }: { entry: LawEntry }) {
...
function StatusBadge({ entry }: { entry: LawEntry }) {
  ...
  if (entry.status === "overruled") {
    return (
      <span className="text-[10px] px-1.5 py-0.5 rounded bg-rose-950/60 text-rose-300 font-mono border border-rose-900/60">OVERRULED</span>
    );
  }
...
export default function LawPage() {
  ...
  const bySection: Record<Section, { family: Family; entries: LawEntry[] }[]> = { A: [], B: [], C: [], D: [], E: [] };
  ...
  return (
    <div className="space-y-8">
      <div className="border-b border-gray-800 pb-6">
        <h1 className="text-2xl font-semibold text-white">Law — A Working Taxonomy</h1>
        <p className="mt-3 text-sm text-gray-400 leading-relaxed">
          A field guide to law organized into {ALL_FAMILIES.length} families across five sections — Constitutional &amp;
          Public Law, Criminal Law &amp; Procedure, Civil &amp; Private Law, International &amp; Comparative Law, and
          Legal Theory &amp; Landmark Cases. Each card carries a{" "}
          ...
          <strong>OVERRULED</strong> decisions are preserved here for historical and pedagogical importance.
      ...
      <div className="sticky top-0 z-10 -mx-6 px-6 py-3 bg-gray-950/95 backdrop-blur border-b border-gray-800/60 flex flex-col sm:flex-row gap-3 sm:items-center">
        <input
          ...
          placeholder="Filter by name, key principle, citation, tag — e.g. 'Miranda', 'mens rea', 'Chevron', 'fair use'"
      ...
      <div className="border-t border-gray-800 pt-6 mt-12 space-y-3">
        ...
        <p className="text-xs text-gray-500 leading-relaxed">
          <span className="text-gray-400">Open questions and contested decisions:</span> entries marked{" "}
          <span className="text-red-300 font-mono">OPEN</span> are unresolved as of 2026.{" "}
          <span className="text-amber-300 font-mono">CONTESTED</span> indicates a doctrine where the holding stands
          but is sharply criticized or politically unstable.{" "}
          <span className="text-rose-300 font-mono">OVERRULED</span> entries are preserved here for historical and
          pedagogical reasons (e.g., Lochner, Dred Scott, Korematsu, Chevron). The taxonomy is U.S.-leaning;
          international and comparative sections aim for breadth without claiming completeness. Reports of
          inaccuracy welcome via the{" "}
