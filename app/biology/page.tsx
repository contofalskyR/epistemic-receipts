...
import type { BioEntry, ColorKey, Family, Section } from "./types";
...
const SECTION_INFO: Record<Section, { name: string; tagline: string }> = {
  A: { name: "Section A — Foundations", tagline: "Cell biology, molecular biology, genetics, and evolution — the universal foundations of all living systems." },
  B: { name: "Section B — Core Branches", tagline: "Ecology, physiology, neuroscience, microbiology, development, and immunology — how organisms function and interact." },
  C: { name: "Section C — Organismal Diversity", tagline: "Botany, zoology, fungi, protists, and the classification of life's extraordinary variety." },
  D: { name: "Section D — Applied Biology", tagline: "Biotechnology, conservation, and bioinformatics — biology put to work and held to account." },
  E: { name: "Section E — Open Questions, Contested Classifications & Landmark Discoveries", tagline: "Origins of life, unresolved problems, contested borders, and the experiments that changed everything." },
};

// ────────────────────────────────────────────────────────────────────────────
// Build global name index for graph resolution
// ────────────────────────────────────────────────────────────────────────────

type EntryRef = { entry: BioEntry; family: Family };
...
function entryMatches(entry: BioEntry, query: string): boolean {
  ...
  if (entry.evolvedFrom.some((p) => p.toLowerCase().includes(q))) return true;
  return false;
}

// ────────────────────────────────────────────────────────────────────────────
// Lineage graph (evolved-from DAG)
// ────────────────────────────────────────────────────────────────────────────
...
function layoutFamilyGraph(family: Family): { nodes: GraphNode[]; edges: GraphEdge[] } {
  ...
  for (const entry of family.entries) {
    for (const parent of entry.evolvedFrom) {
  ...
  // Topological level assignment via BFS
    ...
    <div className="overflow-x-auto mb-3">
      <svg width={maxX} height={maxY} style={{ display: "block" }} role="img" aria-label={`Lineage graph for ${family.name}`}>
        ...
        {nodes.map((n) => {
          ...
          const label = n.name.length > 25 ? n.name.slice(0, 24) + "…" : n.name;
          const clickable = n.inFamily;
          return (
            <g key={n.name} onClick={() => { if (clickable) onSelect(n.name); }} style={{ cursor: clickable ? "pointer" : "default" }}>
              <rect
                x={n.x}
                y={n.y}
                width={n.width}
...
function XrefBadges({ entry }: { entry: BioEntry }) {
    ...
    >
      ...
      {expanded && (
        <div className="mt-3 pt-3 -mx-4 -mb-3 px-4 pb-4 border-t border-gray-700/70 bg-gray-900/80 rounded-b space-y-3">
          ...
          {entry.evolvedFrom.length > 0 && (
            <div>
              <p className="text-[10px] uppercase tracking-widest text-gray-500">Evolved from / builds on</p>
              <div className="mt-1 flex flex-wrap gap-1">
                {entry.evolvedFrom.map((p) => {
                  const ref = lookupEntry(p);
                  return ref ? (
                    <span key={p} className={`text-[10px] px-1.5 py-0.5 rounded bg-gray-800 text-gray-300 font-mono`}>{p}</span>
                  ) : (
                    <span key={p} className="text-[10px] px-1.5 py-0.5 rounded bg-gray-900 text-gray-500 font-mono">{p}</span>
                  );
                })}
              </div>
            </div>
...
function FamilySection({
  ...
  function handleGraphSelect(name: string) {
        ...
        <div className="bg-gray-950/40 p-4 space-y-3">
          <div className="flex items-center gap-2">
            <button
              ...
              className={`text-[10px] font-mono px-2 py-1 rounded border ${c.headerBorder} ${c.accent} hover:brightness-125 transition-all`}
            >
              {showGraph ? "hide lineage graph" : "show lineage graph"}
            </button>
            <span className="text-[10px] text-gray-600">arrows = evolved-from / builds-on</span>
          </div>
          {showGraph && (
            <LineageGraph family={family} onSelect={handleGraphSelect} />
          )}
...
export default function BiologyPage() {
  ...
  const bySection: Record<Section, { family: Family; entries: BioEntry[] }[]> = { A: [], B: [], C: [], D: [], E: [] };
  ...
  return (
    <div className="space-y-8">
      <div className="border-b border-gray-800 pb-6">
        <h1 className="text-2xl font-semibold text-white">Biology — A Working Taxonomy</h1>
        <p className="mt-3 text-sm text-gray-400 leading-relaxed">
          A field guide to biology organized into {ALL_FAMILIES.length} families across five sections — Foundations,
          Core Branches, Organismal Diversity, Applied Biology, and Open Questions &amp; Landmark Discoveries.
          Each card carries a <em>key fact</em> and a lineage graph showing which concepts a topic evolved from
          or builds upon. Entries marked <strong>OPEN</strong> are genuinely unresolved; <strong>CONTESTED</strong>{" "}
          marks genuine scientific disputes where reasonable experts disagree. Click a family header to collapse it;
          click a card to expand it; click &ldquo;show lineage graph&rdquo; to reveal the directed concept graph for
          that family (nodes are clickable).
        </p>
        <p className="mt-3 text-xs text-gray-500 leading-relaxed">
          Cross-references: entries link to{" "}
          <Link href="/medicine" className="text-blue-300 underline underline-offset-2 hover:text-blue-200">medicine</Link>,{" "}
          <Link href="/chemistry" className="text-blue-300 underline underline-offset-2 hover:text-blue-200">chemistry</Link>, and{" "}
          ...
          where significant overlap exists, rather than duplicating entries.
        </p>
      ...
      <div className="sticky top-0 z-10 -mx-6 px-6 py-3 bg-gray-950/95 backdrop-blur border-b border-gray-800/60 flex flex-col sm:flex-row gap-3 sm:items-center">
        <input
          ...
          placeholder="Filter by name, key fact, tag — e.g. 'CRISPR', 'mitochondria', 'natural selection', 'open'"
      ...
      {filtered.length === 0 ? (
        ...
        <div className="space-y-8">
      <FieldGuideBanner domain="Biology" className="mb-2" />
      ...
      <div className="border-t border-gray-800 pt-6 mt-12 space-y-3">
        ...
        <p className="text-xs text-gray-500 leading-relaxed">
          ...
          <span className="text-red-300 font-mono">OPEN</span> are genuinely unresolved as of 2026.{" "}
          <span className="text-amber-300 font-mono">CONTESTED</span> entries reflect genuine scientific disputes
          — reasonable experts disagree. Notable contested areas: whether viruses are alive, the species concept,
          the functional fraction of the genome (ENCODE debate), the extent of epigenetic inheritance across
          generations, and the scope of horizontal gene transfer in eukaryotes. Volatile facts verified June 2026;
          re-verify extinction statistics, IUCN Red List counts, protected-area percentages, and drug approvals
          before republishing.
        </p>
        <p className="text-xs text-gray-500 leading-relaxed">
          <span className="text-gray-400">Claim cross-references pending.</span> Biology entries will be linked
          to primary literature claims in the main database as that corpus grows.
        </p>
        <p className="text-xs font-mono text-gray-700">
          taxonomy curated 2026-06-04 · {ALL_FAMILIES.length} families · {totalEntries} entries · lineage graph via Edges model
        </p>
      </div>
      <div className="border-t border-gray-700/40 pt-6 mt-4">
        ...
        <div className="flex flex-wrap gap-4">
          <a href="/search?q=biology" className="text-xs text-sky-400/70 hover:text-sky-300 transition-colors font-mono">
            Search Biology in the claim graph →
          </a>
