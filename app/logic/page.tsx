...
import type { ColorKey, Family, LogicEntry, Section } from "./types";
...
const SECTION_INFO: Record<Section, { name: string; tagline: string }> = {
  A: { name: "Section A — Classical Foundations", tagline: "Propositional, predicate, proof-theoretic and model-theoretic foundations of formal logic." },
  B: { name: "Section B — Modal & Multi-Valued Logics", tagline: "Necessity, time, knowledge, obligation, and graded truth — modal and many-valued extensions of the classical core." },
  C: { name: "Section C — Constructive & Substructural", tagline: "Logics built by changing the rules — intuitionistic, substructural, paraconsistent and non-monotonic." },
  D: { name: "Section D — Mathematical Logic", tagline: "Set theory, computability, type theory and categorical logic — the foundational disciplines of mathematics." },
  E: { name: "Section E — Philosophical Logic & Landmarks", tagline: "Vagueness, truth, reference, and the historical arc of logic from Aristotle to Gödel and beyond." },
};
...
// ────────────────────────────────────────────────────────────────────────────
// Build a global index of entries by name (for lineage resolution)
// ────────────────────────────────────────────────────────────────────────────

type EntryRef = { entry: LogicEntry; family: Family };
...
function entryMatches(entry: LogicEntry, query: string): boolean {
  if (!query) return true;
  const q = query.toLowerCase();
  if (entry.name.toLowerCase().includes(q)) return true;
  if (entry.description.toLowerCase().includes(q)) return true;
  if (plainText(entry.formalDef).toLowerCase().includes(q)) return true;
  if (plainText(entry.keyResults).toLowerCase().includes(q)) return true;
  ...
  if (entry.extends.some((p) => p.toLowerCase().includes(q))) return true;
  return false;
}

// ────────────────────────────────────────────────────────────────────────────
// Lineage DAG — per family graph layout (uses extends field)
// ────────────────────────────────────────────────────────────────────────────
...
function layoutFamilyGraph(family: Family): { nodes: GraphNode[]; edges: GraphEdge[] } {
  ...
  for (const entry of family.entries) {
    for (const parent of entry.extends) {
      const ref = lookupEntry(parent);
      if (ref) {
        if (ref.family.slug === family.slug) {
          edges.push({ from: parent, to: entry.name });
          allNodeNames.add(parent);
        } else {
...
function LineageGraph({ family, onSelect }: { family: Family; onSelect: (name: string) => void }) {
  ...
  return (
    <div className="overflow-x-auto mb-3">
      <svg width={maxX} height={maxY} style={{ display: "block" }} role="img" aria-label={`Lineage graph for ${family.name}`}>
        <defs>
          <marker
            id={markerId}
            markerWidth="8"
      ...
      <p className="text-[10px] text-gray-500 mt-1">
        ...
        primitive / cross-family ancestor
      </p>
    </div>
  );
}
...
function HistoricalBadge({ entry }: { entry: LogicEntry }) {
  ...
  return (
    <span className="text-[10px] px-1.5 py-0.5 rounded font-mono bg-gray-800/80 text-gray-300 border border-gray-700">
      HISTORICAL
    </span>
  );
}

function XrefBadges({ entry }: { entry: LogicEntry }) {
  ...
  return (
    ...
    >
      ...
      <div className="mt-2 text-xs text-gray-300 leading-relaxed">
        <span className="text-[10px] uppercase tracking-widest text-gray-500 mr-2">Formal def</span>
        <MathFragment text={entry.formalDef} />
      </div>

      {expanded && (
        <div className="mt-3 pt-3 -mx-4 -mb-3 px-4 pb-4 border-t border-gray-700/70 bg-gray-900/80 rounded-b space-y-3">
          <div>
            <p className="text-[10px] uppercase tracking-widest text-gray-500">Key results</p>
            <p className="mt-1 text-xs text-gray-300 leading-relaxed">
              <MathFragment text={entry.keyResults} />
            </p>
          </div>
          {entry.notation && (
            <div>
              <p className="text-[10px] uppercase tracking-widest text-gray-500">Notation</p>
              <p className="mt-1 text-xs text-gray-300 leading-relaxed">
                <MathFragment text={entry.notation} />
              </p>
            </div>
          )}
          <div>
            <p className="text-[10px] uppercase tracking-widest text-gray-500">Extends</p>
            {entry.extends.length === 0 ? (
              <p className="mt-1 text-xs text-gray-500 italic">Foundational entry — no listed ancestors.</p>
            ) : (
                ...
                {entry.extends.map((p) => {
                  ...
                  if (ref) {
                    return (
                      <li key={p}>
                        <Link
                          href={`/logic#entry-${slugifyEntry(ref.entry.name)}`}
                          onClick={(e) => e.stopPropagation()}
          ...
          {extendedBy.length > 0 && (
            <div>
              <p className="text-[10px] uppercase tracking-widest text-gray-500">Extended by</p>
              <ul className="mt-1 text-xs text-gray-300 leading-relaxed list-disc list-inside">
                {extendedBy.map((name) => {
...
function FamilySection({
  ...
  family: Family;
  filteredEntries: LogicEntry[];
  collapsed: boolean;
  onToggleCollapse: () => void;
  expanded: string | null;
  setExpanded: (key: string | null) => void;
  extendedByIndex: Map<string, string[]>;
}) {
  const c = COLOR_STYLES[family.color];
      ...
      {!collapsed && (
        <div className="bg-gray-950/40 p-4 space-y-3">
          <div className="flex items-center gap-3 text-xs">
            ...
            <span className="text-gray-600">{family.entries.length} total entries · {countEdges(family)} lineage edges</span>
          </div>

          {showGraph && (
            <div className={`rounded border ${c.cardBorder} p-3 bg-gray-900/40`}>
              <p className={`text-[10px] uppercase tracking-widest ${c.accent} mb-2`}>Lineage map — {family.name}</p>
              <LineageGraph
                family={family}
                onSelect={(name) => {
              ...
              return (
                <div key={entry.name} id={`entry-${slugifyEntry(entry.name)}`}>
                  <EntryCard
                    ...
                    extendedBy={extendedByIndex.get(entry.name) ?? []}
                  />
                </div>
              );
            })}
...
function countEdges(family: Family): number {
  let n = 0;
  for (const entry of family.entries) n += entry.extends.length;
  return n;
}
...
// Precompute "extended by" index
function buildExtendedByIndex(families: Family[]): Map<string, string[]> {
  const idx = new Map<string, string[]>();
  for (const fam of families) {
    for (const entry of fam.entries) {
      for (const parent of entry.extends) {
        const ref = lookupEntry(parent);
        const key = ref ? ref.entry.name : parent;
...
export default function LogicPage() {
  ...
  const extendedByIndex = useMemo(() => buildExtendedByIndex(ALL_FAMILIES), []);
  ...
  const bySection: Record<Section, { family: Family; entries: LogicEntry[] }[]> = { A: [], B: [], C: [], D: [], E: [] };
  ...
  return (
    <div className="space-y-8">
      <div className="border-b border-gray-800 pb-6">
        <h1 className="text-2xl font-semibold text-white">Logic — A Working Taxonomy</h1>
        <p className="mt-3 text-sm text-gray-400 leading-relaxed">
          A field guide to logic organised by family — from classical propositional and predicate logic
          through modal, temporal, epistemic, deontic, many-valued, intuitionistic, substructural and
          paraconsistent systems, into set theory, computability, type theory, categorical logic and
          philosophical logic. Each card carries a <em>formal definition</em> (LaTeX, typeset with
          KaTeX), <em>key results</em>, and an <em>extends</em> list — the directed edges of a
          lineage DAG you can toggle on per family. Color codes the section; clicking a header collapses
          it; clicking a card expands it.
        </p>
        <p className="mt-3 text-xs text-gray-500 leading-relaxed">
          Cross-references: foundational results overlap heavily with{" "}
          <Link href="/mathematics" className="text-blue-300 underline underline-offset-2 hover:text-blue-200">mathematics</Link>
          {" "}(model theory, set theory, proof theory),{" "}
          ...
          {" "}(computability, type theory, verification), and{" "}
          <Link href="/philosophy" className="text-blue-300 underline underline-offset-2 hover:text-blue-200">philosophy</Link>
          {" "}(philosophical logic, semantics, paradoxes); entries marked <span className="font-mono">xref</span> link to the
          sibling page rather than duplicating.
        </p>
        <p className="mt-2 text-xs text-gray-500 leading-relaxed">
          Whether logical pluralism, dialetheism, and the epistemicist account of vagueness are correct
          remains <span className="text-amber-400">CONTESTED</span>; landmark resolutions such as
          Cohen&apos;s independence of CH and Friedberg–Muchnik are tagged{" "}
          <span className="text-green-400">SOLVED</span>. Entries from before 1950 are tagged{" "}
          <span className="text-gray-300">HISTORICAL</span>.
        </p>
        <p className="mt-2 text-xs font-mono text-gray-600">
          {ALL_FAMILIES.length} families · {totalEntries} entries · {totalEdges} lineage edges
          {query && (
      ...
      <div className="sticky top-0 z-10 -mx-6 px-6 py-3 bg-gray-950/95 backdrop-blur border-b border-gray-800/60 flex flex-col sm:flex-row gap-3 sm:items-center">
        <input
          ...
          placeholder="Filter by name, definition, key result, tag — e.g. 'Gödel', 'modal', 'compactness', 'Curry–Howard'"
              ...
              <div key={sec} className="space-y-4">
                ...
                {items.map(({ family, entries }) => (
                  <FamilySection
                    ...
                    extendedByIndex={extendedByIndex}
                  />
                ))}
              </div>
            );
      ...
      <div className="border-t border-gray-800 pt-6 mt-12 space-y-3">
        <p className="text-xs text-gray-500 leading-relaxed">
          ...
          the claim is <em>about</em> that concept — only that the term is present. A claim-powered
          explorer that links logical results to the specific receipts that cite them is on the
          roadmap, as is a site-wide lineage map across the full set of entries.
        </p>
        <p className="text-xs text-gray-500 leading-relaxed">
          <span className="text-gray-400">Accuracy note:</span> independence and undecidability
          results are verified against the standard mathematical-logic references — only the
          continuum hypothesis is independent of ZFC (Gödel 1940, Cohen 1963); Robinson&apos;s and
          Church&apos;s 1936 undecidability of first-order logic, Gödel&apos;s 1931 incompleteness
          theorems, Tarski&apos;s 1933 undefinability theorem, Friedberg–Muchnik (1956), and
          Solovay&apos;s arithmetical completeness of GL (1976) are reflected. Open philosophical
          debates (logical pluralism, dialetheism, vagueness) are marked CONTESTED rather than
          adjudicated. Reports of inaccuracy welcome via the{" "}
