...
import type { SocEntry, ColorKey, Family, Section } from "./types";
import { FAMILIES_1_8 } from "./data";
import { FAMILIES_9_16 } from "./data2";
import { FAMILIES_17_23 } from "./data3";

const ALL_FAMILIES: Family[] = [...FAMILIES_1_8, ...FAMILIES_9_16, ...FAMILIES_17_23];
...
const SECTION_INFO: Record<Section, { name: string; tagline: string }> = {
  A: { name: "Section A — Foundations & Theory", tagline: "Classical theorists, paradigms, and contemporary theory." },
  B: { name: "Section B — Social Structure", tagline: "Class, race, gender, family, demography, and organizations." },
  C: { name: "Section C — Culture & Identity", tagline: "Meaning, identity, religion, deviance, networks, and urban life." },
  D: { name: "Section D — Methods & Quantitative Sociology", tagline: "Survey methods, ethnography, statistics, causal inference, and computational social science." },
  E: { name: "Section E — Contested & Open Questions", tagline: "Famous studies, inequality debates, 21st-century frontiers, and open questions." },
};
...
function entryMatches(entry: SocEntry, query: string): boolean {
  ...
  if (entry.theorist && entry.theorist.toLowerCase().includes(q)) return true;
...
function XrefBadges({ entry }: { entry: SocEntry }) {
    ...
    >
      ...
      {entry.theorist && (
        <div className="mt-1 text-[11px] text-gray-400 leading-relaxed">
          <span className="text-[10px] uppercase tracking-widest text-gray-500 mr-2">Theorist</span>
          {entry.theorist}
        </div>
      )}
...
export default function SociologyPage() {
  ...
  const bySection: Record<Section, { family: Family; entries: SocEntry[] }[]> = { A: [], B: [], C: [], D: [], E: [] };
  ...
  return (
    <div className="space-y-8">
      <div className="border-b border-gray-800 pb-6">
        <h1 className="text-2xl font-semibold text-white">Sociology — A Working Taxonomy</h1>
        <p className="mt-3 text-sm text-gray-400 leading-relaxed">
          A field guide to sociology organized into {ALL_FAMILIES.length} families across five sections — Foundations &amp; Theory,
          Social Structure, Culture &amp; Identity, Methods &amp; Quantitative Sociology, and Contested &amp; Open Questions.
          Each card carries a <em>key fact</em>, where relevant a <em>formula</em> (typeset with KaTeX), the original{" "}
          <em>theorist</em>, and an <em>example</em>. Status badges mark <strong>LANDMARK</strong>, <strong>CONTESTED</strong>,{" "}
          <strong>REFUTED</strong>, and <strong>OPEN</strong> entries. The replication crisis hit sociology later and quieter
          than psychology, but several canonical results (Hawthorne, Stanford Prison, broken windows) no longer hold up;
      ...
      <div className="sticky top-0 z-10 -mx-6 px-6 py-3 bg-gray-950/95 backdrop-blur border-b border-gray-800/60 flex flex-col sm:flex-row gap-3 sm:items-center">
        <input
          ...
          placeholder="Filter by name, key fact, formula, tag, theorist — e.g. 'Bourdieu', 'segregation', 'replication'"
      ...
      {filtered.length === 0 ? (
        ...
        <div className="space-y-8">
      <FieldGuideBanner domain="Sociology" className="mb-2" />
      ...
      <div className="border-t border-gray-800 pt-6 mt-12 space-y-3">
        ...
        <p className="text-xs text-gray-500 leading-relaxed">
          ...
          sociology). Reports of inaccuracy welcome via the{" "}
          <Link href="/feedback" className="underline underline-offset-2">feedback</Link> page.
        </p>
        <p className="text-xs font-mono text-gray-700">
          taxonomy curated 2026-06-05 · LaTeX typesetting via KaTeX · {ALL_FAMILIES.length} families · {totalEntries} entries
        </p>
      </div>
      <div className="border-t border-gray-700/40 pt-6 mt-4">
        ...
        <div className="flex flex-wrap gap-4">
          <a href="/search?q=sociology" className="text-xs text-sky-400/70 hover:text-sky-300 transition-colors font-mono">
            Search Sociology in the claim graph →
          </a>
