      ...
      <div className="flex flex-col space-y-5 border-b border-gray-800 pb-10">
        ...
        <div className="rounded-md border border-gray-800/60 bg-gray-900/40 px-4 py-3 space-y-1.5">
          ...
          <ul className="space-y-1 text-xs text-gray-500">
            <li><span className="text-gray-400">/physiology taxonomy page — 18 families, 162 entries</span> — new sibling of <span className="font-mono">/neuroscience</span> / <span className="font-mono">/biology</span> / <span className="font-mono">/medicine</span> / <span className="font-mono">/chemistry</span> at <span className="font-mono">/physiology</span>. Five sections (Cellular &amp; Molecular Foundations · Cardiovascular &amp; Respiratory · Renal, GI, Endocrine &amp; Reproductive · Musculoskeletal, Immune &amp; Hematology · Integration, Aging &amp; Frontiers) cover <strong>18 families · 162 entries</strong>, threaded by ten organ systems (cardiovascular, respiratory, nervous, endocrine, digestive, renal, musculoskeletal, immune, reproductive, cellular). Each card carries a KaTeX-typeset <span className="font-mono">keyFact</span>, optional <span className="font-mono">formula</span> (Nernst, GHK, Fick, Starling, Poiseuille, Frank-Starling, alveolar-gas, etc.), an <span className="font-mono">organSystem</span> tag, status badges (LANDMARK / CONTESTED / OPEN), and <span className="font-mono">xref</span> chips into <span className="font-mono">neuroscience</span>, <span className="font-mono">medicine</span>, <span className="font-mono">biology</span>, <span className="font-mono">chemistry</span>. Live Research panel wired via <span className="font-mono">GET /api/taxonomy/physiology/stats</span>. Data layer split <span className="font-mono">app/physiology/&#123;types,data,data2,data3,page&#125;.tsx</span>; counts computed from the data, not hardcoded. <span className="font-mono">/fields</span> hub updated under Natural Sciences between Neuroscience and Astronomy. <span className="font-mono">tsc --noEmit</span> clean.</li>
            <li><span className="text-gray-400">OpenAlex live research panels — taxonomy pages linked to live claims</span> — eight taxonomy pages (<span className="font-mono">/neuroscience</span>, <span className="font-mono">/psychology</span>, <span className="font-mono">/chemistry</span>, <span className="font-mono">/medicine</span>, <span className="font-mono">/biology</span>, <span className="font-mono">/physics</span>, <span className="font-mono">/mathematics</span>, <span className="font-mono">/anthropology</span>) now show a <strong>Live Research</strong> card at the bottom of each page. The card fetches live claim counts and 3 sample paper titles from the DB (via <span className="font-mono">GET /api/taxonomy/[slug]/stats</span>) and links to the full claims view at <span className="font-mono">/topics/[slug]</span>. The ingest script (<span className="font-mono">scripts/ingest-openalex.ts</span>) gained six new topic-aligned buckets — <strong>neuroscience</strong> (C54355233 / C2522767166), <strong>biology</strong> (C86803240 / C184235292), <strong>physics</strong> (C121332964 / C62520636), <strong>mathematics</strong> (C33923547), <strong>anthropology</strong> (C142362112), <strong>economics</strong> (C162324750) — each creating a <span className="font-mono">Topic</span> slug matching the taxonomy page path. The existing <strong>cognition</strong> bucket was also updated to co-tag claims with the <span className="font-mono">neuroscience</span> topic (cognitive science and neuroscience overlap heavily). New shared component <span className="font-mono">components/LiveResearchCard.tsx</span>; new API route <span className="font-mono">app/api/taxonomy/[slug]/stats/route.ts</span>. Cards render silently (return null) until the ingest buckets are run against the DB. <span className="font-mono">tsc --noEmit</span> clean.</li>
            <li><span className="text-gray-400">link audit fixes — WHO GHO sources, OpenAlex prefix, /sports nav</span> — three link-audit follow-ups applied. (1) <strong>WHO GHO source URLs</strong>: WHO restructured indicator detail pages to use kebab-case title slugs instead of indicator codes, breaking all 5 <span className="font-mono">who_gho_v1</span> source URLs (and therefore the &ldquo;source&rdquo; link on all 1,001 downstream claims). New script <span className="font-mono">scripts/backfill-who-gho-urls.ts</span> walks each WHO GHO source, maps the indicator code segment to its new slug (e.g. <span className="font-mono">WHOSIS_000001 → life-expectancy-at-birth-(years)</span>), HEAD-verifies each new URL returns 200, then updates <span className="font-mono">Source.url</span> in place; the ingester (<span className="font-mono">scripts/ingest-who-gho.ts</span>) now carries a <span className="font-mono">detailSlug</span> field on each indicator so future re-runs don&apos;t regress. Verified all 5 new URLs HTTP 200. (2) <strong>OpenAlex bare-domain URL</strong>: one OpenAlex source row had <span className="font-mono">www.lingref.com/cpp/acal/35/paper1298.pdf</span> with no scheme (would be treated as relative by browsers); prepended <span className="font-mono">https://</span> after confirming the canonical URL returns 200. (3) <strong>/sports orphan page</strong>: 859-line Sports &amp; Sport Science taxonomy (19 families, 6 sections) had no nav inbound; added as a sibling under <strong>Applied &amp; Professional</strong> on <span className="font-mono">/fields</span> next to Governance. <span className="font-mono">tsc --noEmit</span> clean.</li>
            <li><span className="text-gray-400">/globe — category density layers</span> — the 3D globe at <span className="font-mono">/globe</span> gains a third dimension on top of the existing <strong>Heatmap</strong> / <strong>Origins</strong> toggle. A second row of pill chips — <strong>All · Science · Law · Legislation · Medicine · Government · History</strong> — sits under the toggle (heatmap mode only) and filters the country heatmap to one thematic layer. Clicking a chip resets the time slider to present, then fetches <span className="font-mono">GET /api/globe/density?category=&lt;slug&gt;</span>; the density route now accepts an optional <span className="font-mono">?category=</span> param, validates the slug against <span className="font-mono">CATEGORY_SLUGS</span>, narrows the pipeline-by-<span className="font-mono">ingestedBy</span> aggregate to that category&apos;s whitelist, and skips the <span className="font-mono">PoliticalContext</span> sweep (same rationale as <span className="font-mono">density-temporal</span>: source-country mixing would dilute the category total). Unknown slugs fall back to the all-claims behavior, so the route stays backward-compatible. Category-to-pipeline mapping lives in new shared lib <span className="font-mono">lib/globe-categories.ts</span> (one source of truth, typed <span className="font-mono">CategorySlug</span>): <strong>Science</strong> = OpenAlex / NIH Reporter / ClinicalTrials / NASA Exoplanet / USGS / space missions / volcanic eruptions / Nobel / PubChem / NIST / PDG / impact craters / nuclear tests / periodic table / GenBank / IAU constellations / solar system / Retraction Watch / MeSH; <strong>Law</strong> = ECHR / WIPO / ICJ / ICC / WTO / CourtListener (SCOTUS · Circuits · State Supreme · BIA · Tax · Disclosures) / AfricanLII / African Court; <strong>Legislation</strong> = ~85 national parliaments + EU + congress trackers; <strong>Medicine</strong> = openFDA labels / FAERS / ChEBI / RxNorm / OMIM / ICD-11 / WHO GHO / WHO essential medicines / FDA aesthetic devices; <strong>Government</strong> = OFAC SDN / World Bank / SIPRI MILEX / FRED / UN SC/GA/treaties / NATO / FARA / OpenFEC / Federal Register / SEC EDGAR / Miller Center; <strong>History</strong> = NARA / JACAR / Europeana WWI / UK National Archives / FRUS / Taiwan archives / Romania CNSAS / Czech ABS / Stasi / LoC / CIA FOIA / JFK / IPN / CWGC. To make the Law and Science layers actually render meaningful heat, <span className="font-mono">lib/globe-pipeline-country.ts</span> picked up unambiguous US-only pipeline mappings that were previously absent (CourtListener federal/state courts, OpenFEC, FARA, NIH Reporter, ClinicalTrials, NASA Exoplanet, PubChem, GenBank, OMIM, NIST, openFDA, congress trackers); the existing &ldquo;multi-country&rdquo; omissions (USGS earthquakes, CrossRef retractions, OpenAlex, UN, EU, WHO, etc.) stay omitted because attributing them to one country would be misleading. Client integration in <span className="font-mono">app/globe/GlobeClient.tsx</span>: a new <span className="font-mono">categoryFilter</span> state nullable to a <span className="font-mono">CategorySlug</span>, the existing year-density effect now handles three cases (at-present + no-category → SSR cache; at-present + category → category API; historical → temporal API, ignoring category for v1), and the slider footer reads <span className="font-mono">&ldquo;N claims in &lt;Category&gt;&rdquo;</span> when a chip is active. The origins mode, time slider, country sidebar, and connections sub-page are untouched. <span className="font-mono">npx tsc --noEmit</span> clean.</li>
            <li><span className="text-gray-400">/feed — the &ldquo;What&apos;s new&rdquo; feed</span> — new page at <span className="font-mono">/feed</span> consolidating recent site activity into four sections. <strong>Since your last visit</strong> (client island) reads <span className="font-mono">er_last_visit</span> from <span className="font-mono">localStorage</span>, calls <span className="font-mono">GET /api/feed/new-since?since=&lt;iso&gt;</span> for a count of new claims, then rewrites the timestamp to now; first-time visitors get a welcome state instead of a count. <strong>Recent claims by pipeline</strong> (server-fetched) does a Prisma <span className="font-mono">groupBy</span> on <span className="font-mono">ingestedBy</span> over the last 7 days, ranks the top 6 pipelines by volume, and renders each as a card with the pipeline tag, the claim count, and 3 snippet links (~80 chars) — each pipeline name deep-links to <span className="font-mono">/claims?ingestedBy=&lt;tag&gt;</span>. <strong>Recent threshold events</strong> (server-fetched) lists the 10 newest <span className="font-mono">ThresholdEvent</span> rows from the last 7 days, each linking to the claim detail page with the <span className="font-mono">triggeredBy</span> source and date. <strong>Activity on your bookmarks</strong> (client island) reads <span className="font-mono">er_profile_key</span> from <span className="font-mono">localStorage</span> and calls <span className="font-mono">GET /api/feed/bookmarked-activity?key=&lt;hash&gt;</span>, which hashes the key (same SHA-256 pattern as <span className="font-mono">/api/bookmarks</span>), looks up the profile&apos;s <span className="font-mono">Bookmark</span> rows, and returns up to 20 bookmarked claims with any threshold events in the last 30 days; no-profile / no-bookmarks / no-activity each get their own empty state instead of crashing. New files: <span className="font-mono">app/feed/page.tsx</span> (server component), <span className="font-mono">app/feed/SinceLastVisit.tsx</span>, <span className="font-mono">app/feed/BookmarkedActivity.tsx</span>, <span className="font-mono">app/api/feed/new-since/route.ts</span>, <span className="font-mono">app/api/feed/bookmarked-activity/route.ts</span>. Nav link added between Stats and Bookmarks; no schema change. <span className="font-mono">tsc --noEmit</span> clean.</li>
