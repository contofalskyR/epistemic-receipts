...
type Lens = "curated" | "machine";

const LENS_COPY: Record<Lens, { label: string; blurb: string }> = {
  curated: {
    label: "Curated",
    blurb:
      "Every trajectory tracks one historical claim's dated, sourced status changes across five communities — expert literature, institutions, courts, the public, and markets. Browse by era, search the claims, and open any one to trace its settling curve.",
  },
  machine: {
    label: "Machine-derived",
    blurb:
      "The most notable curves generated by the data pipelines — ranked by reversals, transition count, and time span. Not hand-curated: each card links to the underlying receipt with its sources. Same-day certification pairs are excluded.",
  },
};
...
export default function TrajectoryEncyclopedia() {
  const [lens, setLens] = useState<Lens>("curated");
  ...
  const data = responses[lens] ?? null;

  // One fetch per lens, cached in state so toggling back is instant. Each lens
  ...
  return (
    <div style={{ background: C.bg, color: C.ink, minHeight: "100%" }} className="w-full">
      <div className="mx-auto px-5 py-8" style={{ maxWidth: 1100 }}>
        ...
        <p className="mb-4" style={{ color: C.mut, fontSize: 13.5, maxWidth: 680, lineHeight: 1.5 }}>
          {LENS_COPY[lens].blurb}
        </p>

        {/* Lens toggle: hand-curated trajectories vs. notable machine-derived curves */}
        <div className="flex gap-2 mb-4">
          {(["curated", "machine"] as Lens[]).map((l) => {
            const on = l === lens;
            return (
              <button
                key={l}
              ...
              >
                {LENS_COPY[l].label}
                {responses[l] ? ` · ${responses[l]!.eraCounts?.all ?? responses[l]!.total}` : ""}
              </button>
            );
          })}
        </div>
            ...
            <div className="grid gap-3 grid-cols-1 sm:grid-cols-2 lg:grid-cols-3">
              {pageItems.map((it) => (
                <Link
                  ...
                  href={
                    it.kind === "machine"
                      ? `/claims/${encodeURIComponent(it.id)}`
                      : `/settling-curve?t=${encodeURIComponent(it.id)}`
                  }
