...
export default function PartyEconomicPanel({ global, us }: Props) {
  ...
  // Coverage % computed from the dataset itself — never hardcode this; the
  // JSON is regenerated by scripts and a literal value silently goes stale.
  const coveragePct =
    data.coverage && data.coverage.totalInScopeLaws > 0
      ? ((data.coverage.lawsWithParty / data.coverage.totalInScopeLaws) * 100).toFixed(1)
      : null;

  const computedDate = data.generatedAt ? data.generatedAt.slice(0, 10) : null;
  ...
  return (
    <section className="space-y-3 scroll-mt-4">
      ...
      <p className="text-[11px] text-gray-600 italic max-w-3xl leading-relaxed">
        Party attribution covers {coveragePct ? `${coveragePct}%` : "a subset"} of
        legislation in scope
        {computedDate && (
          <span className="not-italic font-mono"> (computed {computedDate})</span>
        )}
        . Global dataset includes EU national laws; US filter isolates congressional votes only.
