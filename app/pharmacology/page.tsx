...
> = {
  ...
  gray:    { headerBg: "bg-gray-900/40",    headerBorder: "border-gray-700",    headerText: "text-gray-200",    chipBg: "bg-gray-800/60",    chipText: "text-gray-300",    cardBorder: "border-gray-800/70",    cardHover: "hover:border-gray-600",    accent: "text-gray-400" },
};

const SECTION_INFO: Record<Section, { name: string; tagline: string }> = {
  A: { name: "Section A — Pharmacokinetics (ADME) & Pharmacodynamics", tagline: "What the body does to the drug (PK) and what the drug does to the body (PD) — the quantitative core." },
  B: { name: "Section B — Drug Classes by Body System (I): Nervous System, Analgesia & Anesthesia", tagline: "ATC group N and analgesic/anesthetic classes." },
  C: { name: "Section C — Drug Classes by Body System (II): Cardiovascular, Respiratory & GI", tagline: "ATC groups C, R, A." },
  D: { name: "Section D — Drug Classes by Body System (III): Endocrine, Anti-infectives & Antineoplastics", tagline: "ATC groups A, H, G, J, P, L." },
  E: { name: "Section E — Toxicology, Blood, Dermatology & Sensory", tagline: "ATC groups B, D, S, plus clinical toxicology." },
  F: { name: "Section F — Drug Development, Regulation & Pharmacogenomics", tagline: "How drugs are discovered, tested, approved, monitored, and personalized — the meta-layer." },
  G: { name: "Section G — Foundations & Cross-cutting Concepts", tagline: "Definitions and organizing ideas that thread the whole field." },
};
...
function entryMatches(entry: PharmEntry, query: string): boolean {
  if (!query) return true;
  const q = query.toLowerCase();
  if (entry.name.toLowerCase().includes(q)) return true;
  if (entry.description.toLowerCase().includes(q)) return true;
  if (entry.mechanism && entry.mechanism.toLowerCase().includes(q)) return true;
  if (entry.indications && entry.indications.toLowerCase().includes(q)) return true;
  if (entry.formula && plainText(entry.formula).toLowerCase().includes(q)) return true;
  if (entry.className && entry.className.toLowerCase().includes(q)) return true;
  if (entry.example && plainText(entry.example).toLowerCase().includes(q)) return true;
  if (entry.cautions && entry.cautions.toLowerCase().includes(q)) return true;
  if (entry.atc?.some((a) => a.toLowerCase().includes(q))) return true;
...
function XrefBadges({ entry }: { entry: PharmEntry }) {
...
function StatusBadge({ entry }: { entry: PharmEntry }) {
  if (!entry.status) return null;
  if (entry.status === "needs-verification") {
    return (
      <span className="text-[10px] px-1.5 py-0.5 rounded bg-amber-950/60 text-amber-300 font-mono border border-amber-900/60">VERIFY</span>
    );
  }
  if (entry.status === "in-flux") {
    return (
      <span className="text-[10px] px-1.5 py-0.5 rounded bg-yellow-950/60 text-yellow-300 font-mono border border-yellow-900/60">IN FLUX</span>
    );
  }
  return (
    <span className="text-[10px] px-1.5 py-0.5 rounded bg-red-950/60 text-red-300 font-mono border border-red-900/60">CONTESTED</span>
  );
}

function KindBadge({ kind }: { kind: PharmEntry["kind"] }) {
  if (kind === "drug") {
    return (
      <span className="text-[10px] px-1.5 py-0.5 rounded bg-blue-950/50 text-blue-300 font-mono">drug</span>
    );
  }
  return (
    <span className="text-[10px] px-1.5 py-0.5 rounded bg-violet-950/50 text-violet-300 font-mono">concept</span>
  );
}

// ────────────────────────────────────────────────────────────────────────────
// ATC tree (collapsed reference)
// ────────────────────────────────────────────────────────────────────────────

function AtcReference() {
  return (
    <section className="rounded-lg border border-gray-700 overflow-hidden">
      <div className="px-5 py-3 bg-gray-900/40 border-b border-gray-800">
        <h2 className="text-base font-semibold text-gray-200">WHO ATC Classification — Main Groups</h2>
        <p className="text-xs text-gray-500 mt-0.5">
          The 14 anatomical main groups (Level 1) of the WHO Anatomical Therapeutic Chemical system.
          ATC codes on drug entries below position each class in this tree.
        </p>
      </div>
      <div className="p-4 bg-gray-950/40">
        <div className="grid gap-2 sm:grid-cols-2">
          {ATC_MAIN_GROUPS.map((g) => (
            <div key={g.code} className="flex items-baseline gap-2 text-xs">
              <span className="font-mono text-blue-400 w-5 shrink-0">{g.code}</span>
...
function EntryCard({
  ...
  return (
    ...
    >
      ...
      {(entry.tags.length > 0 || (entry.xref && entry.xref.length > 0) || (entry.atc && entry.atc.length > 0)) && (
        <div className="mt-2 flex flex-wrap gap-1 items-center">
          {entry.atc?.map((a) => (
            <span key={a} className="text-[10px] px-1.5 py-0.5 rounded bg-blue-950/40 text-blue-300 font-mono border border-blue-900/40">
              ATC {a}
            </span>
          ))}
          {entry.tags.map((tag) => (
      ...
      {entry.mechanism && (
        <div className="mt-2 text-xs text-gray-300 leading-relaxed">
          <span className="text-[10px] uppercase tracking-widest text-gray-500 mr-2">Mechanism</span>
          <MathFragment text={entry.mechanism} />
        </div>
      )}
      {entry.formula && (
        <div className="mt-2 text-xs text-gray-300 leading-relaxed">
      ...
      {expanded && (
        <div className="mt-3 pt-3 -mx-4 -mb-3 px-4 pb-4 border-t border-gray-700/70 bg-gray-900/80 rounded-b space-y-3">
          {entry.indications && (
            <div>
              <p className="text-[10px] uppercase tracking-widest text-gray-500">Indications</p>
              <p className="mt-1 text-xs text-gray-300 leading-relaxed">{entry.indications}</p>
            </div>
          )}
          {entry.example && (
            <div>
          ...
          {entry.schedule && (
            <div>
              <p className="text-[10px] uppercase tracking-widest text-gray-500">Schedule / governance</p>
              <p className="mt-1 text-xs text-gray-300 leading-relaxed">{entry.schedule}</p>
            </div>
          )}
          {entry.cautions && (
            <div>
              <p className="text-[10px] uppercase tracking-widest text-amber-400/80">Cautions / critiques</p>
...
export default function PharmacologyPage() {
  ...
  const bySection: Record<Section, { family: Family; entries: PharmEntry[] }[]> = {
    A: [], B: [], C: [], D: [], E: [], F: [], G: [],
  };
  ...
  return (
    <div className="space-y-8">
      <div className="border-b border-gray-800 pb-6">
        <h1 className="text-2xl font-semibold text-white">Pharmacology — A Working Taxonomy</h1>
        <p className="mt-3 text-sm text-gray-400 leading-relaxed">
          A field guide to pharmacology organized into {ALL_FAMILIES.length} families across seven sections —
          from pharmacokinetics and pharmacodynamics, through drug classes organized by body system
          (CNS, cardiovascular, respiratory, GI, endocrine, anti-infectives, antineoplastics), toxicology
          and hematology, drug development and regulation, pharmacogenomics, to the foundational classificatory
          concepts. <strong>Drug entries</strong> carry a <em>Mechanism</em>, <em>Indications</em>,
          and WHO <em>ATC codes</em>; <strong>concept entries</strong> carry a{" "}
          <em>Definition/formula</em> (LaTeX, typeset with KaTeX). Color codes the section;
          clicking a header collapses it; clicking a card expands it.
        </p>
        <p className="mt-3 text-xs text-gray-500 leading-relaxed">
          <strong>Responsible scope:</strong> this taxonomy is reference-level and descriptive.
          Drug classes, mechanisms, indications, adverse-effect classes, ATC codes, and scheduling are in scope.
          Synthesis routes, misuse dosing, and operational detail for controlled substances are not.
        </p>
        <p className="mt-3 text-xs text-gray-500 leading-relaxed">
          ...
          <Link href="/public-health" className="text-emerald-300 underline underline-offset-2 hover:text-emerald-200">public health</Link>,{" "}
      ...
      <div className="sticky top-0 z-10 -mx-6 px-6 py-3 bg-gray-950/95 backdrop-blur border-b border-gray-800/60 flex flex-col sm:flex-row gap-3 sm:items-center">
        <input
          ...
          placeholder="Filter by name, mechanism, indication, ATC code, formula, tag — e.g. 'SSRI', 'beta-blocker', 'N05A', 'bioavailability'"
      ...
      <LiveResearchCard slug="pharmacology" />
      ...
      <div className="border-t border-gray-800 pt-6 mt-12 space-y-3">
        <p className="text-xs text-gray-500 leading-relaxed">
          ...
          search over claim and source text. ATC codes link drug classes to the WHO system;
          a term appearing in a claim does not mean the claim is <em>about</em> that drug — only that the term is present.
        </p>
